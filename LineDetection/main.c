...
int main(){
    ...
    const char *outputFileVer = "images/lena512_VerLines.bmp";
    const char *outputFileHor = "images/lena512_HorLines.bmp";
    const char *outputFilelDiag = "images/lena512_lDiagLines.bmp";
    const char *outputFilerDiag = "images/lena512_rDiagLines.bmp";
    ...
    // Step 2: Create 3x3 convolution masks for edge detection
    // Each mask highlights edges in a specific direction

    // Vertical edge detection mask
    mask* verticalMask = maskCreate(3, 3);
    if(!verticalMask || !verticalMask->data){
        fprintf(stderr, "Error: Could not allocate mask.\n");
        BMP8Free(image);
        return 1;
    }
    // Fill the vertical mask
    for(int j = 0; j < verticalMask->cols; j++){
        for(int i = 0; i < verticalMask->rows; i++){
            if(j == 0 || j == 2){
                verticalMask->data[i * verticalMask->cols + j] = -1;
            }
            else{
                verticalMask->data[i * verticalMask->cols + j] = 2;
            }
        }
    }

    // Horizontal edge detection mask
    mask* horizontalMask = maskCreate(3, 3);
    if(!horizontalMask || !horizontalMask->data){
        fprintf(stderr, "Error: Could not allocate mask.\n");
        BMP8Free(image);
        return 1;
    }
    // Fill the horizontal mask
    for(int j = 0; j < horizontalMask->cols; j++){
        for(int i = 0; i < horizontalMask->rows; i++){
            if(i == 0 || i == 2){
                horizontalMask->data[i * horizontalMask->cols + j] = -1;
            }
            else{
                horizontalMask->data[i * horizontalMask->cols + j] = 2;
            }
        }
    }

    // Left diagonal edge detection mask
    mask* leftDiagonalMask = maskCreate(3, 3);
    if(!leftDiagonalMask || !leftDiagonalMask->data){
        fprintf(stderr, "Error: Could not allocate mask.\n");
        BMP8Free(image);
        return 1;
    }
    // Fill the left diagonal mask
    for(int j = 0; j < leftDiagonalMask->cols; j++){
        for(int i = 0; i < leftDiagonalMask->rows; i++){
            if(i == j){
                leftDiagonalMask->data[i * leftDiagonalMask->cols + j] = 2;
            }
            else{
                leftDiagonalMask->data[i * leftDiagonalMask->cols + j] = -1;
            }
        }
    }

    // Right diagonal edge detection mask
    mask* rightDiagonalMask = maskCreate(3, 3);
    if (!rightDiagonalMask || !rightDiagonalMask->data) {
        fprintf(stderr, "Error: Could not allocate mask.\n");
        BMP8Free(image);
        return 1;
    }
    // Fill the right diagonal mask
    for (int j = 0; j < rightDiagonalMask->cols; j++) {
        for (int i = 0; i < rightDiagonalMask->rows; i++) {
            if (i + j == rightDiagonalMask->cols - 1) {
                rightDiagonalMask->data[i * rightDiagonalMask->cols + j] = 2;
            } else {
                rightDiagonalMask->data[i * rightDiagonalMask->cols + j] = -1;
            }
        }
    }

    // Step 3: Apply convolution with each directional mask
    BMP8Image* convolvedVer = BMP8Convolution(image, verticalMask);
    BMP8Image* convolvedHor = BMP8Convolution(image, horizontalMask);
    BMP8Image* convolvedLeftDiag = BMP8Convolution(image, leftDiagonalMask);
    BMP8Image* convolvedRightDiag = BMP8Convolution(image, rightDiagonalMask);

    // Step 4: Save the convolved images
    BMP8save(outputFileVer, convolvedVer);
    BMP8save(outputFileHor, convolvedHor);
    BMP8save(outputFilelDiag, convolvedLeftDiag);
    BMP8save(outputFilerDiag, convolvedRightDiag);

    // Step 5: Free all allocated memory
    BMP8Free(image);
    BMP8Free(convolvedVer);
    BMP8Free(convolvedHor);
    BMP8Free(convolvedLeftDiag);
    BMP8Free(convolvedRightDiag);

    maskFree(verticalMask);
    maskFree(horizontalMask);
    maskFree(leftDiagonalMask);
    maskFree(rightDiagonalMask);

    printf("Convolution completed! Saved results.\n");
    return 0;
}